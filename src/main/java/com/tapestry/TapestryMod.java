 ...
 * 
 * This class orchestrates complete Tapestry lifecycle including
...
public class TapestryMod implements ModInitializer {
    ...
    /**
     * Phase 1: Bootstrap core framework.
     */
    ...
    private static void registerFabricHooks() {
        ...
        ServerLifecycleEvents.SERVER_STARTED.register(server -> {
            ...
            // For integrated servers, this will be handled by client-side flag check
        ...
        // Register this regardless of environment type because integrated servers need it too
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            ...
            if (schedulerService != null) {
                try {
                    LOGGER.info("Ticking scheduler on server thread with tick: {}", server.getTicks());
                    schedulerService.tick(server.getTicks());
                } catch (Exception e) {
                    LOGGER.error("Error during scheduler tick", e);
                }
            } else {
                LOGGER.warn("Scheduler service is null during server tick");
            }
            
            // Handle client-side initialization check (combined from second handler)
    ...
    private static void loadTypeScriptMods() {
        ...
        // Freeze registries and get API tree for TypeScript
        apiRegistry.freeze();
        hookRegistry.freeze();
        ...
        try {
            ...
            for (DiscoveredMod mod : discoveredMods) {
                ...
                // Use same registry as mod.define() function
        ...
        try {
            ...
            // Use same config directory as other services (ConfigService)
        ...
        // Extend tapestry object with hook APIs for TS_READY phase
    ...
    /**
     * Extracts source name from a file path.
     * 
    ...
    /**
     * Gets TapestryAPI instance.
     * This is primarily for testing and internal use.
     * 
     * @return TapestryAPI instance
     */
    ...
    /**
     * Gets TypeScriptRuntime instance.
     * This is primarily for testing and internal use.
     * 
     * @return TypeScriptRuntime instance
     */
    ...
    /**
     * Gets global EventBus instance for Phase 11 reactive event system.
     * 
     * @return EventBus instance or null if not initialized
     */
     ...
     * 
     * @param apiRegistry API registry to register functions to
     */
    ...
    /**
     * Initializes Phase 10.5 mod system with two-pass loading.
     */
